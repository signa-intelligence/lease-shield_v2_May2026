...
// REMOVED: import NotificationPreferences from "../components/settings/NotificationPreferences";
...
function NotificationPreferences({ user, onUpdate, colors }) {
  const [saving, setSaving] = useState(false);
  const [preferences, setPreferences] = useState({
    email_notifications: user?.email_notifications ?? true,
    line_notifications: user?.line_notifications ?? false,
    deposit_alerts: user?.deposit_alerts ?? true,
    deposit_alert_30d: user?.deposit_alert_30d ?? true,
    deposit_alert_7d: user?.deposit_alert_7d ?? true,
    deposit_alert_3d: user?.deposit_alert_3d ?? true,
    deposit_alert_overdue: user?.deposit_alert_overdue ?? true,
    lease_notices: user?.lease_notices ?? true,
    lease_notice_30d: user?.lease_notice_30d ?? true,
    lease_notice_7d: user?.lease_notice_7d ?? true,
    lease_notice_3d: user?.lease_notice_3d ?? true,
    lease_notice_0d: user?.lease_notice_0d ?? true,
    ...
    quiet_hours_timezone: user?.quiet_hours_timezone ?? 'Asia/Bangkok'
  });
  ...
  const strings = {
    en: {
      ...
      timeSelectionError: 'End time must be after start time for quiet hours to be effective.'
    },
    th: {
      ...
      timeSelectionError: 'เวลาสิ้นสุดต้องหลังจากเวลาเริ่มต้นเพื่อให้ช่วงเวลาเงียบมีผล'
    },
    zh: {
      title: '通知偏好设置',
      ...
      connected: '已连接',
      notConnected: '未连接',
      timeSelectionError: '安静时间结束时间必须在开始时间之后才能生效。'
    },
    ja: {
      title: '通知設定',
      ...
      connected: '接続済み',
      notConnected: '未接続',
      timeSelectionError: 'サイレントアワーを有効にするには、終了時刻が開始時刻より後である必要があります。'
    },
    ko: {
      title: '알림 설정',
      ...
      timeSelectionError: '방해 금지 시간을 효과적으로 사용하려면 종료 시간이 시작 시간보다 늦어야 합니다.'
    }
  };
  ...
  return (
    <Card className="border-none shadow-xl" style={{ backgroundColor: colors.cardBg }}>
      <CardHeader className="border-b pb-4" style={{
        ...
        <p className="text-sm mt-1" style={{ color: colors.textSecondary }}>{str.subtitle}</p>
      </CardHeader>
      <CardContent className="p-6">
        <form onSubmit={handleSubmit} className="space-y-6">
          {/* Notification Channels */}
          <div>
            <h3 className="font-semibold mb-3 text-lg" style={{ color: colors.textPrimary }}>{str.channels}</h3>
            <div className="grid gap-4">
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <Label htmlFor="email-notifications" className="font-medium" style={{ color: colors.textPrimary }}>
                      {str.email}
                    </Label>
                    <p className="text-xs" style={{ color: colors.textSecondary }}>{str.allEnabled}</p>
                  </div>
                </div>
                <div
                  className={`w-10 h-6 flex items-center p-1 rounded-full cursor-pointer transition-colors duration-200 ${preferences.email_notifications ? 'bg-ls-forest' : 'bg-gray-300'}`}
                  onClick={() => handleToggle('email_notifications')}
                  style={{
                    backgroundColor: preferences.email_notifications ? '#0C3B2E' : (isDarkMode ? '#4B5563' : '#E5E7EB')
                  }}
                >
                  <div
                    className={`bg-white w-4 h-4 rounded-full shadow-md transform transition-transform duration-200 ${preferences.email_notifications ? 'translate-x-4' : 'translate-x-0'}`}
                  />
                </div>
              </div>
              ...
              <div
                ...
                style={{
                  backgroundColor: colors.fieldBg,
                  borderColor: colors.borderColor,
                  ...(user?.line_user_id ? {} : { opacity: hasLineAccess ? 1 : 0.6 })
                }}
              >
                <div className="flex items-center gap-3">
                  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-5 h-5 text-ls-forest">
                    <path fillRule="evenodd" d="M12 2.25c-5.385 0-9.75 4.365-9.75 9.75s4.365 9.75 9.75 9.75 9.75-4.365 9.75-9.75S17.385 2.25 12 2.25Zm-2.625 6.75a.75.75 0 1 0 0 1.5h4.75a.75.75 0 0 0 0-1.5h-4.75Zm0 2.25a.75.75 0 1 0 0 1.5h4.75a.75.75 0 0 0 0-1.5h-4.75Zm-2.25 2.25a.75.75 0 1 0 0 1.5h6.75a.75.75 0 0 0 0-1.5H7.125Z" clipRule="evenodd" />
                  </svg>

                  <div>
                    <Label className="font-medium" style={{ color: colors.textPrimary }}>
                      {str.line}
                    </Label>
                    <div className="flex items-center gap-2">
                      <span className="text-xs" style={{ color: colors.textSecondary }}>
                        {user?.line_user_id ? str.connected : str.notConnected}
                      </span>
                      {user?.line_user_id ? (
                ...
                {hasLineAccess && !user?.line_user_id ? (
                  <Button size="sm" onClick={() => window.open(createPageUrl('ConnectLine'), '_blank')}>
                    {str.connectLine}
                  </Button>
                ) : (
                  <div
                    className={`w-10 h-6 flex items-center p-1 rounded-full transition-colors duration-200 ${preferences.line_notifications && hasLineAccess ? 'bg-ls-forest' : 'bg-gray-300'} ${hasLineAccess && user?.line_user_id ? 'cursor-pointer' : 'cursor-not-allowed'}`}
                    onClick={() => hasLineAccess && user?.line_user_id && handleToggle('line_notifications')}
                    style={{
                      backgroundColor: preferences.line_notifications && hasLineAccess ? '#0C3B2E' : (isDarkMode ? '#4B5563' : '#E5E7EB')
                    }}
                  >
                    <div
                      className={`bg-white w-4 h-4 rounded-full shadow-md transform transition-transform duration-200 ${preferences.line_notifications && hasLineAccess ? 'translate-x-4' : 'translate-x-0'}`}
                    />
                  </div>
                )}
              </div>
              {!hasLineAccess && (
                <div className="text-right text-xs mt-1" style={{ color: colors.textSecondary }}>
                  <Badge className="mr-1 bg-ls-gold/20 text-ls-gold">{str.premiumFeature}</Badge>
                  {str.requiresPremium}
                </div>
              )}
            </div>
          ...
          {/* Notification Types */}
          <div>
            <h3 className="font-semibold mb-3 text-lg" style={{ color: colors.textPrimary }}>{str.types}</h3>
            <div className="space-y-4">
              <div className="p-4 rounded-xl border" style={{ backgroundColor: colors.fieldBg, borderColor: colors.borderColor }}>
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-3">
                    <Coins className="w-5 h-5 text-ls-gold" />
                    <Label htmlFor="deposit-alerts" className="font-medium" style={{ color: colors.textPrimary }}>
                      {str.depositAlerts}
                    </Label>
                  </div>
                  <div
                    className={`w-10 h-6 flex items-center p-1 rounded-full cursor-pointer transition-colors duration-200 ${preferences.deposit_alerts ? 'bg-ls-forest' : 'bg-gray-300'}`}
                    onClick={() => handleToggle('deposit_alerts')}
                    style={{
                      backgroundColor: preferences.deposit_alerts ? '#0C3B2E' : (isDarkMode ? '#4B5563' : '#E5E7EB')
                    }}
                  >
                    <div
                      className={`bg-white w-4 h-4 rounded-full shadow-md transform transition-transform duration-200 ${preferences.deposit_alerts ? 'translate-x-4' : 'translate-x-0'}`}
                    />
                  </div>
                </div>
                {preferences.deposit_alerts && (
                  <div className="grid grid-cols-2 gap-x-4 gap-y-2 text-sm mt-3 border-t pt-3" style={{ borderColor: colors.borderColor }}>
                    <label className="flex items-center justify-between col-span-2 sm:col-span-1">
                      <span style={{ color: colors.textSecondary }}>{str.deposit30d}</span>
                      <div
                        className={`w-10 h-6 flex items-center p-1 rounded-full cursor-pointer transition-colors duration-200 ${preferences.deposit_alert_30d ? 'bg-ls-forest' : 'bg-gray-300'}`}
                        onClick={() => handleToggle('deposit_alert_30d')}
                        style={{
                          backgroundColor: preferences.deposit_alert_30d ? '#0C3B2E' : (isDarkMode ? '#4B5563' : '#E5E7EB')
                        }}
                      >
                        <div
                          className={`bg-white w-4 h-4 rounded-full shadow-md transform transition-transform duration-200 ${preferences.deposit_alert_30d ? 'translate-x-4' : 'translate-x-0'}`}
                        />
                      </div>
                    </label>
                    <label className="flex items-center justify-between col-span-2 sm:col-span-1">
                      <span style={{ color: colors.textSecondary }}>{str.deposit7d}</span>
                      <div
                        className={`w-10 h-6 flex items-center p-1 rounded-full cursor-pointer transition-colors duration-200 ${preferences.deposit_alert_7d ? 'bg-ls-forest' : 'bg-gray-300'}`}
                        onClick={() => handleToggle('deposit_alert_7d')}
                        style={{
                          backgroundColor: preferences.deposit_alert_7d ? '#0C3B2E' : (isDarkMode ? '#4B5563' : '#E5E7EB')
                        }}
                      >
                        <div
                          className={`bg-white w-4 h-4 rounded-full shadow-md transform transition-transform duration-200 ${preferences.deposit_alert_7d ? 'translate-x-4' : 'translate-x-0'}`}
                        />
                      </div>
                    </label>
                    <label className="flex items-center justify-between col-span-2 sm:col-span-1">
                      <span style={{ color: colors.textSecondary }}>{str.deposit3d}</span>
                      <div
                        className={`w-10 h-6 flex items-center p-1 rounded-full cursor-pointer transition-colors duration-200 ${preferences.deposit_alert_3d ? 'bg-ls-forest' : 'bg-gray-300'}`}
                        onClick={() => handleToggle('deposit_alert_3d')}
                        style={{
                          backgroundColor: preferences.deposit_alert_3d ? '#0C3B2E' : (isDarkMode ? '#4B5563' : '#E5E7EB')
                        }}
                      >
                        <div
                          className={`bg-white w-4 h-4 rounded-full shadow-md transform transition-transform duration-200 ${preferences.deposit_alert_3d ? 'translate-x-4' : 'translate-x-0'}`}
                        />
                      </div>
                    </label>
                    <label className="flex items-center justify-between col-span-2 sm:col-span-1">
                      <span style={{ color: colors.textSecondary }}>{str.depositOverdue}</span>
                      <div
                        className={`w-10 h-6 flex items-center p-1 rounded-full cursor-pointer transition-colors duration-200 ${preferences.deposit_alert_overdue ? 'bg-ls-forest' : 'bg-gray-300'}`}
                        onClick={() => handleToggle('deposit_alert_overdue')}
                        style={{
                          backgroundColor: preferences.deposit_alert_overdue ? '#0C3B2E' : (isDarkMode ? '#4B5563' : '#E5E7EB')
                        }}
                      >
                        <div
                          className={`bg-white w-4 h-4 rounded-full shadow-md transform transition-transform duration-200 ${preferences.deposit_alert_overdue ? 'translate-x-4' : 'translate-x-0'}`}
                        />
                      </div>
                    </label>
                  </div>
              ...
              <div className="p-4 rounded-xl border" style={{ backgroundColor: colors.fieldBg, borderColor: colors.borderColor }}>
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-3">
                    ...
                    <Label htmlFor="lease-notices" className="font-medium" style={{ color: colors.textPrimary }}>
                      {str.leaseNotices}
                    </Label>
                  </div>
                  <div
                    className={`w-10 h-6 flex items-center p-1 rounded-full cursor-pointer transition-colors duration-200 ${preferences.lease_notices ? 'bg-ls-forest' : 'bg-gray-300'}`}
                    onClick={() => handleToggle('lease_notices')}
                    style={{
                      backgroundColor: preferences.lease_notices ? '#0C3B2E' : (isDarkMode ? '#4B5563' : '#E5E7EB')
                    }}
                  >
                    <div
                      className={`bg-white w-4 h-4 rounded-full shadow-md transform transition-transform duration-200 ${preferences.lease_notices ? 'translate-x-4' : 'translate-x-0'}`}
                    />
                  </div>
                </div>
                {preferences.lease_notices && (
                  <div className="grid grid-cols-2 gap-x-4 gap-y-2 text-sm mt-3 border-t pt-3" style={{ borderColor: colors.borderColor }}>
                    <label className="flex items-center justify-between col-span-2 sm:col-span-1">
                      <span style={{ color: colors.textSecondary }}>{str.lease30d}</span>
                      <div
                        className={`w-10 h-6 flex items-center p-1 rounded-full cursor-pointer transition-colors duration-200 ${preferences.lease_notice_30d ? 'bg-ls-forest' : 'bg-gray-300'}`}
                        onClick={() => handleToggle('lease_notice_30d')}
                        style={{
                          backgroundColor: preferences.lease_notice_30d ? '#0C3B2E' : (isDarkMode ? '#4B5563' : '#E5E7EB')
                        }}
                      >
                        <div
                          className={`bg-white w-4 h-4 rounded-full shadow-md transform transition-transform duration-200 ${preferences.lease_notice_30d ? 'translate-x-4' : 'translate-x-0'}`}
                        />
                      </div>
                    </label>
                    <label className="flex items-center justify-between col-span-2 sm:col-span-1">
                      <span style={{ color: colors.textSecondary }}>{str.lease7d}</span>
                      <div
                        className={`w-10 h-6 flex items-center p-1 rounded-full cursor-pointer transition-colors duration-200 ${preferences.lease_notice_7d ? 'bg-ls-forest' : 'bg-gray-300'}`}
                        onClick={() => handleToggle('lease_notice_7d')}
                        style={{
                          backgroundColor: preferences.lease_notice_7d ? '#0C3B2E' : (isDarkMode ? '#4B5563' : '#E5E7EB')
                        }}
                      >
                        <div
                          className={`bg-white w-4 h-4 rounded-full shadow-md transform transition-transform duration-200 ${preferences.lease_notice_7d ? 'translate-x-4' : 'translate-x-0'}`}
                        />
                      </div>
                    </label>
                    <label className="flex items-center justify-between col-span-2 sm:col-span-1">
                      <span style={{ color: colors.textSecondary }}>{str.lease3d}</span>
                      <div
                        className={`w-10 h-6 flex items-center p-1 rounded-full cursor-pointer transition-colors duration-200 ${preferences.lease_notice_3d ? 'bg-ls-forest' : 'bg-gray-300'}`}
                        onClick={() => handleToggle('lease_notice_3d')}
                        style={{
                          backgroundColor: preferences.lease_notice_3d ? '#0C3B2E' : (isDarkMode ? '#4B5563' : '#E5E7EB')
                        }}
                      >
                        <div
                          className={`bg-white w-4 h-4 rounded-full shadow-md transform transition-transform duration-200 ${preferences.lease_notice_3d ? 'translate-x-4' : 'translate-x-0'}`}
                        />
                      </div>
                    </label>
                    <label className="flex items-center justify-between col-span-2 sm:col-span-1">
                      <span style={{ color: colors.textSecondary }}>{str.lease0d}</span>
                      <div
                        className={`w-10 h-6 flex items-center p-1 rounded-full cursor-pointer transition-colors duration-200 ${preferences.lease_notice_0d ? 'bg-ls-forest' : 'bg-gray-300'}`}
                        onClick={() => handleToggle('lease_notice_0d')}
                        style={{
                          backgroundColor: preferences.lease_notice_0d ? '#0C3B2E' : (isDarkMode ? '#4B5563' : '#E5E7EB')
                        }}
                      >
                        <div
                          className={`bg-white w-4 h-4 rounded-full shadow-md transform transition-transform duration-200 ${preferences.lease_notice_0d ? 'translate-x-4' : 'translate-x-0'}`}
                        />
                      </div>
                    </label>
                  </div>
              ...
              <div className="flex items-center justify-between p-4 rounded-xl border" style={{ backgroundColor: colors.fieldBg, borderColor: colors.borderColor }}>
                <div className="flex items-center gap-3">
                  <Shield className="w-5 h-5 text-ls-forest" />
                  <div>
                    <Label htmlFor="rent-reminders" className="font-medium" style={{ color: colors.textPrimary }}>
                      {str.rentReminders}
                    </Label>
                    <p className="text-xs" style={{ color: colors.textSecondary }}>{str.rentReminderDesc}</p>
                  </div>
                </div>
                <div
                  className={`w-10 h-6 flex items-center p-1 rounded-full cursor-pointer transition-colors duration-200 ${preferences.rent_reminders ? 'bg-ls-forest' : 'bg-gray-300'}`}
                  onClick={() => handleToggle('rent_reminders')}
                  style={{
                    backgroundColor: preferences.rent_reminders ? '#0C3B2E' : (isDarkMode ? '#4B5563' : '#E5E7EB')
                  }}
                >
                  <div
                    className={`bg-white w-4 h-4 rounded-full shadow-md transform transition-transform duration-200 ${preferences.rent_reminders ? 'translate-x-4' : 'translate-x-0'}`}
                  />
                </div>
              </div>
              ...
              <div className="flex items-center justify-between p-4 rounded-xl border" style={{ backgroundColor: colors.fieldBg, borderColor: colors.borderColor }}>
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <Label htmlFor="maintenance-updates" className="font-medium" style={{ color: colors.textPrimary }}>
                      {str.maintenanceUpdates}
                    </Label>
                    <p className="text-xs" style={{ color: colors.textSecondary }}>{str.maintenanceDesc}</p>
                  </div>
                </div>
                <div
                  className={`w-10 h-6 flex items-center p-1 rounded-full cursor-pointer transition-colors duration-200 ${preferences.maintenance_updates ? 'bg-ls-forest' : 'bg-gray-300'}`}
                  onClick={() => handleToggle('maintenance_updates')}
                  style={{
                    backgroundColor: preferences.maintenance_updates ? '#0C3B2E' : (isDarkMode ? '#4B5563' : '#E5E7EB')
                  }}
                >
                  <div
                    className={`bg-white w-4 h-4 rounded-full shadow-md transform transition-transform duration-200 ${preferences.maintenance_updates ? 'translate-x-4' : 'translate-x-0'}`}
                  />
                </div>
              </div>
              ...
              <div className="flex items-center justify-between p-4 rounded-xl border" style={{ backgroundColor: colors.fieldBg, borderColor: colors.borderColor }}>
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <Label htmlFor="case-updates" className="font-medium" style={{ color: colors.textPrimary }}>
                      {str.caseUpdates}
                    </Label>
                    <p className="text-xs" style={{ color: colors.textSecondary }}>{str.caseDesc}</p>
                  </div>
                </div>
                <div
                  className={`w-10 h-6 flex items-center p-1 rounded-full cursor-pointer transition-colors duration-200 ${preferences.case_updates ? 'bg-ls-forest' : 'bg-gray-300'}`}
                  onClick={() => handleToggle('case_updates')}
                  style={{
                    backgroundColor: preferences.case_updates ? '#0C3B2E' : (isDarkMode ? '#4B5563' : '#E5E7EB')
                  }}
                >
                  <div
                    className={`bg-white w-4 h-4 rounded-full shadow-md transform transition-transform duration-200 ${preferences.case_updates ? 'translate-x-4' : 'translate-x-0'}`}
                  />
                </div>
              </div>
              ...
              <div className="flex items-center justify-between p-4 rounded-xl border" style={{ backgroundColor: colors.fieldBg, borderColor: colors.borderColor }}>
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <Label htmlFor="marketing-emails" className="font-medium" style={{ color: colors.textPrimary }}>
                      {str.marketing}
                    </Label>
                    <p className="text-xs" style={{ color: colors.textSecondary }}>{str.marketingDesc}</p>
                  </div>
                </div>
                <div
                  className={`w-10 h-6 flex items-center p-1 rounded-full cursor-pointer transition-colors duration-200 ${preferences.marketing_emails ? 'bg-ls-forest' : 'bg-gray-300'}`}
                  onClick={() => handleToggle('marketing_emails')}
                  style={{
                    backgroundColor: preferences.marketing_emails ? '#0C3B2E' : (isDarkMode ? '#4B5563' : '#E5E7EB')
                  }}
                >
                  <div
                    className={`bg-white w-4 h-4 rounded-full shadow-md transform transition-transform duration-200 ${preferences.marketing_emails ? 'translate-x-4' : 'translate-x-0'}`}
                  />
                </div>
              </div>
            </div>
          ...
          {/* Quiet Hours */}
          <div>
            <h3 className="font-semibold mb-3 text-lg" style={{ color: colors.textPrimary }}>{str.quietHours}</h3>
            <div className="p-4 rounded-xl border" style={{ backgroundColor: colors.fieldBg, borderColor: colors.borderColor }}>
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-3">
                  <BellOff className="w-5 h-5 text-gray-500" /> {/* BellOff icon for quiet hours */}
                  <div>
                    <Label htmlFor="quiet-hours-enabled" className="font-medium" style={{ color: colors.textPrimary }}>
                      {str.enable} {str.quietHours}
                    </Label>
                    <p className="text-xs" style={{ color: colors.textSecondary }}>{str.quietHoursDesc}</p>
                  </div>
                </div>
                <div
                  className={`w-10 h-6 flex items-center p-1 rounded-full cursor-pointer transition-colors duration-200 ${preferences.quiet_hours_enabled ? 'bg-ls-forest' : 'bg-gray-300'}`}
                  onClick={() => handleToggle('quiet_hours_enabled')}
                  style={{
                    backgroundColor: preferences.quiet_hours_enabled ? '#0C3B2E' : (isDarkMode ? '#4B5563' : '#E5E7EB')
                  }}
                >
                  <div
                    className={`bg-white w-4 h-4 rounded-full shadow-md transform transition-transform duration-200 ${preferences.quiet_hours_enabled ? 'translate-x-4' : 'translate-x-0'}`}
                  />
                </div>
              </div>
              {preferences.quiet_hours_enabled && (
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-3 border-t pt-3" style={{ borderColor: colors.borderColor }}>
                  <div>
                    <Label htmlFor="quiet-hours-start" style={{ color: colors.textPrimary }}>{str.startTime}</Label>
                    <Select
                  ...
                  <div>
                    <Label htmlFor="quiet-hours-end" style={{ color: colors.textPrimary }}>{str.endTime}</Label>
                    <Select
                  ...
                  <div className="col-span-1 sm:col-span-2">
                    <Label htmlFor="quiet-hours-timezone" style={{ color: colors.textPrimary }}>{str.timezone}</Label>
                    <Select
                      value={preferences.quiet_hours_timezone}
                      onValueChange={(value) => handleTimeChange('quiet_hours_timezone', value)}
                    >
                      <SelectTrigger className="mt-2" style={{ backgroundColor: colors.inputBg, borderColor: colors.borderColor, color: colors.textPrimary }}>
                        <SelectValue />
                      </SelectTrigger>
          ...
          <div className="flex justify-end pt-4">
            <Button type="submit" className="bg-ls-forest hover:bg-ls-forest/90" disabled={saving}>
              <Save className="w-4 h-4 mr-2" />
              {saving ? str.saving : str.saveChanges}
            </Button>
          </div>
        </form>
      </CardContent>
...
export default function Profile() {
  ...
  React.useEffect(() => {
    ...
    if (hash === '#notifications') {
      setTimeout(() => {
        const notificationSection = document.getElementById('notification-analytics'); // Changed to notification-preferences as per rendering below
