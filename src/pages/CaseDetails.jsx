...
function CaseDetailsContent() {
  ...
  // Upload state — mirrors DocumentVault pattern
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      setCustomLabel('');
      setSelectedFiles([]);
      toast.success(language === 'th' ? 'อัปโหลดสำเร็จ' : 'Upload successful');
      haptic.success();
    } catch (error) {
      console.error('Upload failed:', error);
      toast.error(language === 'th' ? 'อัปโหลดไม่สำเร็จ โปรดลองอีกครั้ง' : 'Upload failed. Please try again.');
      haptic.error();
    } finally {
      setUploading(false);
    }
  ...
  return (
    <div className="min-h-screen p-4 md:p-6 page-transition" style={{ backgroundColor: colors.bg }}>
      <div className="max-w-5xl mx-auto">
        ...
        {/* Upload Section — inline always-visible (mirrors DocumentVault) */}
        <Card className="border-none shadow-xl" style={{ backgroundColor: colors.cardBg }}>
          <CardHeader style={{ borderBottom: `1px solid ${colors.borderColor}` }} className="p-4 md:p-6">
          ...
          <CardContent className="p-4 md:p-6">
            <div className="space-y-4">
              <div>
                <Label htmlFor="case_doc_type" style={{ color: colors.textPrimary }}>
                  {language === 'th' ? 'ประเภทเอกสาร' : 'Document Type'}
                </Label>
                <Select value={uploadType} onValueChange={setUploadType}>
                  <SelectTrigger id="case_doc_type" className="mt-2" style={{ backgroundColor: colors.cardBg, borderColor: colors.borderColor, color: colors.textPrimary }}>
                    <SelectValue />
                  </SelectTrigger>
              ...
              <div>
                <Label htmlFor="case_custom_label" style={{ color: colors.textPrimary }}>
                  {language === 'th' ? 'ป้ายกำกับที่กำหนดเอง' : 'Custom Label'}
                </Label>
                <Input
                  id="case_custom_label"
                  type="text"
                  value={customLabel}
                  onChange={(e) => setCustomLabel(e.target.value)}
                  placeholder={language === 'th' ? 'เช่น รูปภาพตอนย้ายเข้า' : 'e.g., Move-in photos'}
                  className="mt-2"
                  style={{ backgroundColor: colors.cardBg, borderColor: colors.borderColor, color: colors.textPrimary }}
                />
              </div>
              ...
              <div>
                <input
                  ...
                  disabled={uploading}
                />
                <label htmlFor="case-file-upload" className={uploading ? 'cursor-not-allowed opacity-70' : ''}>
                  <div
                  ...
                  >
                    ...
                    <p className="font-semibold mb-1 text-sm md:text-base" style={{ color: colors.textPrimary }}>
                      {language === 'th' ? 'ลากและวางไฟล์ที่นี่หรือคลิกเพื่อเลือก' : 'Drag & drop files here or click to browse'}
                    </p>
                    <p className="text-xs md:text-sm" style={{ color: colors.textSecondary }}>
                      {language === 'th' ? 'PDF, รูปภาพ (JPG, PNG), วิดีโอ (MP4, MOV, AVI)' : 'PDF, Images (JPG, PNG), Videos (MP4, MOV, AVI)'}
                    </p>
                  </div>
                </label>
              </div>
