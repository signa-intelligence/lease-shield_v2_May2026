...
Deno.serve(async (req) => {
  try {
    ...
    if (!isAdmin) {
      console.warn('[adminModifyCredits] Non-admin attempted access:', admin.email);
      return Response.json({ success: false, error: 'Admin access required' }, { status: 403 });
    }

    const { userId, action, amount, reason, adminUserId, adminEmail } = await req.json();
    ...
    console.log('[adminModifyCredits] Request:', {
      userId,
      action,
      amount,
      reason,
    ...
    const user = users[0];
    const previousBalance = user.letter_credits || 0;
    ...
    console.log('[adminModifyCredits] Balance change:', {
    ...
    await base44.asServiceRole.entities.CreditLedger.create({
      ...
      admin_user_id: adminUserId || admin.id,
      admin_email: adminEmail || admin.email
    });

    console.log('[adminModifyCredits] Success:', {
      userId,
      previousBalance,
      newBalance
    });
    ...
    console.error('[adminModifyCredits] ERROR:', error.message, error.stack);
    return Response.json({ 
      success: false, 
      error: error.message 
    }, { status: 500 });
