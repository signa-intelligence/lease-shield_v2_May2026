...
Deno.serve(async (req) => {
  try {
    ...
    // Helper to send notification (LINE Flex primary, email fallback)
    const sendNotification = async (user, messageText, subject, flexMessage = null) => {
    ...
    for (const deposit of deposits) {
      ...
      if (daysDiff === 30) {
        ...
        if (await sendNotification(user, messageText, subject, flexMessage)) {
