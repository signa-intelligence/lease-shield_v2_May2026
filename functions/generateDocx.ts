...
Deno.serve(async (req) => {
  try {
    ...
    docSections.push(
      ...
      new Paragraph({
        text: 'Generated by Lease Shield | www.leaseshield.asia',
        alignment: AlignmentType.CENTER,
        spacing: { after: 100 }
      }),
      new Paragraph({
        text: `Multi-language letter pack for ${recipientType} - ${languagePack.allLanguages.length} languages included`,
        alignment: AlignmentType.CENTER,
        spacing: { after: 200 }
      }),
      new Paragraph({
        children: [
          new TextRun({
            text: 'This letter is AI-assisted and human-reviewed. Please verify all details before sending.',
            size: 18,
            italics: true,
            color: '94a3b8'
          })
