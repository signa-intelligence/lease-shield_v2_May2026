...
// ENGINE 5: RIGHTS SUPPRESSION + USE RESTRICTIONS EXPANDED
...
OUTPUT FORMAT:
{
  ...
  "missing_safeguards": [
    ...
    try {
      ...
      const llm2 = llmRes.data?.issues || [];
      const normalized = [];
      llm2.forEach(i => {
        try {
          const emitted = emitIssue(i, { leaseId, scanId });
          if (emitted) normalized.push(emitted);
          else {
            invalidIssues.push({ rule_id: i.rule_id || 'UNKNOWN_RULE', reason: 'schema_validation_failed' });
            invalidDetails.push(collectInvalid(i, i.rule_id));
          }
        } catch (e) {
          invalidIssues.push({ rule_id: i.rule_id || 'UNKNOWN_RULE', reason: `exception:${e.message}` });
        }
      });
      combinedIssues = combinedIssues.concat(normalized);
    } catch (e) {
    ...
    return Response.json({
      ...
      validation: {
        ...
        invalid_rule_ids: invalidDetails.map(d=>d.issue_preview?.rule_id).filter(Boolean)
      },
      llm_layer: {
